...
Global $bigpic, $blackjpg, $blurb, $bytes, $caption, $category, $cd, $cdkey, $cdkeys, $cease, $changelogs, $check, $checklist, $checksum
Global $checkval, $cnt, $compare, $completed, $cookie, $cookies, $cover, $covers, $covimg, $datafold, $DBfile, $declare, $descript
Global $descriptions, $dest, $details, $DetailsGUI, $DLC, $dlcfile, $done, $down, $downfiles, $downlist, $download, $downloads, $downlog
Global $drv, $endgame, $entries, $entry, $erred, $existDB, $exists, $extbin, $extdmg, $extexe, $extpkg, $extsh, $extzip, $f, $file
Global $fileinfo, $filepth, $files, $filesize, $final, $flag, $fold, $found, $free, $game, $gamefold, $gamelist, $gamepic, $games
Global $gamesfold, $gamesini, $gametxt, $gams, $getlatest, $gmefold, $gmesfld, $gogcli, $GOGcliGUI, $hash, $head, $height, $histfile
Global $history, $htmlfle, $i, $icoD, $icoF, $icoI, $icoS, $icoT, $icoW, $icoX, $ID, $identry, $ignore, $image, $imgfle, $include, $ind
Global $inifle, $json, $keep, $key, $lang, $left, $line, $lines, $link, $list, $listcheck, $listed, $listview, $log, $logfle, $lowid
Global $m, $manall, $manalt, $manifest, $manifests, $manlist, $md5check, $mini, $minimize, $model, $n, $name, $num, $numb, $offline, $OP
Global $open, $OS, $OSes, $outfold, $overlook, $params, $part, $parts, $percent, $ping, $pinged, $processed, $progress, $pth, $purge, $r
Global $rat, $ratify, $read, $record, $relax, $reportexe, $res, $rest, $results, $ret, $return, $row, $s, $same, $savkeys, $savlog, $second
Global $selector, $session, $SetupGUI, $shell, $size, $slug, $slugF, $slugfld, $space, $splash, $state, $stop, $style, $subfold, $SYS, $tag
...
Func MainGUI()
	...
	Local $get, $hours, $IDD, $idlink, $ids, $l, $language, $languages, $last, $lastgame, $lastone, $latest, $loop, $manfold
	Local $mans, $method, $mins, $mpos, $nmb, $OPS, $orange, $orphans, $outline, $p, $patchfld, $paths, $pos, $prior, $proceed
	...
	$Item_downall_info = GUICtrlCreateMenuItem("Info", $Sub_menu_downall)
	GUICtrlCreateMenuItem("", $Sub_menu_downall)
	GUICtrlCreateMenuItem("", $Sub_menu_downall)
		...
		Case $msg = $Button_down Or $valnow = 1
			...
			Else
				...
				If $ctrl = False Then
					$shift = _IsPressed("10")
				EndIf
				If $buttxt = "DOWNLOAD" & @LF & "ALL" And $valnow = "" And $down = "" Then
					$allgames = 1
					ContinueLoop
				ElseIf $ctrl = True And $down = "" And ($buttxt <> "VALIDATE" & @LF & "GAME" And $buttxt <> "VALIDATE" & @LF & "FILE") And $valnow = "" Then
				...
				ElseIf $shift = True And $valnow = "" And ($buttxt = "DOWNLOAD" Or $buttxt = "DOWNLOAD" & @LF & "OTHER") Then
					...
					Else
						GUICtrlSetStyle($Button_down, $BS_MULTILINE)
						GUICtrlSetData($Button_down, "DOWNLOAD" & @LF & "OTHER")
						GUICtrlSetTip($Button_down, "Download a game via another manifest!")
						GUICtrlSetState($Item_verify_file, $GUI_DISABLE)
						GUICtrlSetState($Item_verify_game, $GUI_DISABLE)
						GUICtrlSetState($Item_verify_now, $GUI_DISABLE)
						GUICtrlSetState($Listview_games, $GUI_FOCUS)
						$down = "other"
					EndIf
				ElseIf $buttxt = "DOWNLOAD" & @LF & "LIST" And $valnow = "" And $down = "" And $shift = False Then
											...
											ElseIf StringInStr($line, '"Checksum":') > 0 Then
												...
												If $proceed > 0 Then
													...
													If IniRead($downfiles, $col4, "file", "") <> $col4 Then
														If $IDD <> "" And $titleD <> "" And $URL <> "" Then
					...
					If $alert = 0 Then
						If $ind > -1 Then _GUICtrlListView_ClickItem($Listview_games, $ind, "left", False, 1, 1)
					EndIf
				ElseIf $buttxt = "DOWNLOAD" & @LF & "OTHER" Then
...
;~ 										EndIf
		...
		Case $msg = $Item_downall_start Or $allgames = 1
			...
			; game will be noted as DONE in the DOWNLOAD ALL list. If CANCEL is checked, then a query
			...
			$ans = MsgBox(262177 + 256, "Start Query", "This process can potentially be a lengthy one, and monopolize" _
				...
				& @LF & "NOTE - The 'Cancel' checkbox on the 'Game Files Selector'" _
				& @LF & "window can be used to stop 'Downloading ALL', after any" _
				& @LF & "current download has finished.", 0, $GOGcliGUI)
			If $ans = 1 Then
				If FileExists($cookies) Then
					$res = _FileReadToArray($cookies, $array)
					If $res = 1 Then
				...
				If $allgames = 1 Then
					If FileExists($alldown) Then
						...
						If @error = 0 Then
							...
							GUICtrlSetData($Label_top, "Download ALL Started")
							_FileWriteLog($logfle, "Using DOWNLOAD ALL - " & $lastgame & " games.", -1)
							$cease = StringSplit($stop, " ", 1)
							$cease = $cease[1]
							$completed = 0
							$delay = StringSplit($rest, " ", 1)
							...
							For $a = 1 To $lastgame
								...
								WinSetTitle($GOGcliGUI, "", $Scriptname & " - " & $title)
								$slug = IniRead($gamesini, $ID, "slug", "")
								;MsgBox(262192, "Selected Entry", "Title = " & $title & @LF & "ID = " & $ID & @LF & "Slug = " & $slug _
								;	& @LF & @LF & "(auto close in 3 seconds)", 3, $GOGcliGUI)
								GUICtrlSetData($Label_mid, "Connecting to GOG")
								GUICtrlSetData($Label_bed, "Manifest Retrieval")
								$ping = Ping("gog.com", 4000)
								If $ping > 0 Then
									_FileWriteLog($logfle, "Game Title - " & $title, -1)
									_FileWriteLog($logfle, "Downloading latest MANIFEST entry.", -1)
									GUICtrlSetData($Label_mid, "Adding Game to Manifest")
									If $minimize = 1 Then
										$flag = @SW_MINIMIZE
									Else
									...
									If $erred = 0 And $game <> "" Then
										...
										If $processed = $endgame Then
											...
											_FileWriteLog($logfle, "Games processed = " & $completed, -1)
											_FileWriteLog($logfle, "Sessions processed = " & $session, -1)
											If $session = $cease Then
												; Stop
												ExitLoop
											Else
												; Pause
												GUICtrlSetData($Label_mid, "RESTING")
												GUICtrlSetData($Label_bed, $rest)
												WinSetTitle($GOGcliGUI, "", $Scriptname)
												Sleep($delay)
												$processed = 0
												$session = $session + 1
											EndIf
									...
									EndIf
								Else
									_FileWriteLog($logfle, "No web connection.", -1)
									MsgBox(262192, "Web Error", "No connection detected!", 0, $GOGcliGUI)
									ExitLoop
								EndIf
							Next
							WinSetTitle($GOGcliGUI, "", $Scriptname)
							_FileWriteLog($logfle, "Download ALL Stopped.", -1)
							GUICtrlSetData($Label_top, "Download ALL Stopped")
							FileWriteLine($logfle, "")
							...
							;GUICtrlSetState($Listview_games, $GUI_FOCUS)
							;GUICtrlSetState($Pic_cover, $GUI_FOCUS)
							;_GUICtrlListView_ClickItem($Listview_games, 0, "left", False, 1, 1)
						Else
		...
		Case $msg = $Item_downall_info
			...
			MsgBox(262208, "Process Information", _
				"The DOWNLOAD ALL process is likely to be a lengthy one, especially if" & @LF & _
				"you have a lot of games. As well as that, it is likely to take over many of" & @LF & _
				"your PC resources and the bandwidth of your connection." & @LF & @LF & _
				"It is recommended that you don't do anything else on your PC while it" & @LF & _
				"is engaged in that process." & @LF & @LF & _
				"Due to the nature of this process, your existing Manifest won't be used," & @LF & _
				"other than to add to it or replace an existing game entry. Which means" & @LF & _
				"the process always retrieves the very latest data from GOG, before then" & @LF & _
				"automatically starting the downloading of that listed game." & @LF & @LF & _
				"Normal program settings might override DOWNLOAD ALL options." & @LF & @LF & _
				"With the OPTIONS window for DOWNLOAD ALL, you define what file" & @LF & _
				"types will populate the 'Game Files Selector' list and be downloaded." & @LF & _
				"You also set what other options are used, such as how many games to" & @LF & _
				"download in a row, before then taking a pause (rest) or stopping, and" & @LF & _
				"how long a rest will be. Each group of games is called a session. If you" & @LF & _
				"specify 'Stop After', then the program stops downloading altogether" & @LF & _
				"after the specified session has completed, until you click START again." & @LF & @LF & _
				"After a game's files have completed downloading, then its entry will be" & @LF & _
				"removed from the DOWNLOAD ALL list, and the process automatically" & @LF & _
				"moves onto the next listed game ... subject to options set of course." & @LF & @LF & _
				"A LOG file record is kept of the success of each file downloaded etc. It is" & @LF & _
				"strongly suggested you check the log when you can, and deal with any" & @LF & _
				"listed failures manually. You will be able to sort via the result columns." & @LF & @LF & _
				"NOTES - A download or validation check failure for a file will not halt" & @LF & _
				"the DOWNLOAD ALL process. 'Rests' are used to give your PC and web" & @LF & _
				"connection a break, as well as the GOG server being used. Hard drives" & @LF & _
				"especially, are at risk when doing long periods of writing to them. One" & @LF & _
				"game is downloaded at a time, with the 'Game Files Selector' window" & @LF & _
				"opening and closing for each. Cancel can be used for the process." & @LF & @LF & _
				"Be thoughtful of your impact on GOG and other users.", 0, $GOGcliGUI)
...
;~ 	$caption = "Download ALL - Batman: Arkham Asylum Game of the Year Edition"
;~ 	$caption = "Download ALL - Blitzkrieg: Burning Horizon - Rolling Thunder - Iron Division"
;~ 	$caption = "Download ALL - Close Combat 5: Invasion: Normandy - Utah Beach to Cherbourg"
;~ 	$caption = "Download ALL - Gabriel Knight: Sins of the Fathers – 20th Anniversary Edition"
;~ 	$caption = "Download ALL - Heroes Chronicles [Chapter 2] - Conquest of the Underworld"
;~ 	$caption = "Download ALL - STAR WARS™ - The Force Unleashed™ Ultimate Sith Edition"
;~ 	$caption = "Download ALL - STAR WARS™ Jedi Knight: Dark Forces II - Mysteries of the Sith"
;~ 	$caption = "Download ALL - STAR WARS™ Knights of the Old Republic™ II: The Sith Lords™"
...
;~ 	$caption = "Download ALL - Wallace and Gromit's Episode 1 Fright of the Bumblebees"
	...
	Local $Combo_OSfle, $Combo_shutdown, $Group_exist, $Group_files, $Group_OS, $Group_select, $Label_done, $Label_game, $Label_percent, $Label_shut
	Local $Label_speed, $Label_title, $Label_URL, $Label_warn, $ListView_files, $Progress_bar, $Radio_selall, $Radio_selext, $Radio_selgame, $Radio_selpat
	Local $Radio_selset, $Menu_list, $Sub_menu_remove, $Item_remove_ext, $Item_remove_lin, $Item_remove_mac, $Item_remove_sel, $Item_remove_win
	;
	Local $amount, $began, $begin, $begun, $cancel, $changelog, $checked, $code, $col1, $col2, $col3, $col4, $color, $datapic, $description, $dllcall
	Local $downloading, $dwn, $dwnfle, $edge, $ents, $exist, $fext, $filetit, $finish, $gotten, $hours, $icoDwn, $icofle, $icoUp, $IDD, $idlink, $idx
	Local $imageD, $lastid, $listing, $manfold, $mins, $missing, $movdwn, $movup, $osfle, $paths, $prior, $removed, $saved, $savtxt, $secs, $sect
	Local $sections, $SelectorGUI, $serial, $shutdown, $sizecheck, $skip, $slugD, $speed, $start, $styles, $sum, $taken, $theme, $titleD, $tmpman
	Local $up, $upfle, $val, $valfile, $valid, $visible, $wide
	;
	;$mini = 1
	...
	$ListView_files = GUICtrlCreateListView("||||", 20, 30, $width - 45, 250, $LVS_SHOWSELALWAYS + $LVS_SINGLESEL + $LVS_REPORT + $LVS_NOCOLUMNHEADER, _
	...
	$Label_title = GuiCtrlCreateLabel("", 20, 282, 160, 18, $SS_CENTER + $SS_CENTERIMAGE + $SS_SUNKEN)
	GUICtrlSetBkColor($Label_title, $COLOR_GREEN)
	GUICtrlSetColor($Label_title, $COLOR_WHITE)
	GUICtrlSetResizing($Label_title, $GUI_DOCKAUTO)
	GUICtrlSetTip($Label_title, "File Title!")
	$Label_URL = GuiCtrlCreateLabel("", 182, 282, 221, 18, $SS_CENTER + $SS_CENTERIMAGE + $SS_SUNKEN)
	GUICtrlSetBkColor($Label_URL, 0xFFBB77)
	GUICtrlSetResizing($Label_URL, $GUI_DOCKAUTO)
	GUICtrlSetTip($Label_URL, "Download URL!")
	$Label_game = GuiCtrlCreateLabel("", 405, 282, 160, 18, $SS_CENTER + $SS_CENTERIMAGE + $SS_SUNKEN)
	GUICtrlSetBkColor($Label_game, $COLOR_GREEN)
	GUICtrlSetColor($Label_game, $COLOR_WHITE)
	GUICtrlSetResizing($Label_game, $GUI_DOCKAUTO)
	GUICtrlSetTip($Label_game, "Game Title!")
	;
	...
	Else
		GetFileDownloadDetails($ListView_files)
		$ents = _GUICtrlListView_GetItemCount($ListView_files)
		If StringInStr($caption, "Download ALL -") > 0 Then
			$shutdown = IniRead($inifle, "Download ALL", "shutdown", "")
			If $shutdown = "" Then $shutdown = "none"
			GUICtrlSetData($Combo_shutdown, "none|Hibernate|Logoff|Powerdown|Reboot|Shutdown|Standby", $shutdown)
			If $ents > 0 Then
				$start = 1
	...
	While 1
		$msg = GuiGetMsg()
		Select
		Case $msg = $GUI_EVENT_CLOSE Or $msg = $Button_quit Or $start = "next"
			; Exit / Close / Quit the window
		...
		Case $msg = $Button_dwn
			...
			If IsArray($idx) Then
				...
				EndIf
			EndIf
		Case $msg = $Button_download Or $start = 1
			...
			Else
				...
				If $ping > 0 Or $test = 1 Then
					...
					If $downloading <> "" Then
						...
						For $f = 1 To $files[0]
							...
							If $URL <> "" Then
											...
											Else
												If FileExists($download) Then
													...
													If $down = "other" Then
														...
														If $datafold <> "" Then
															$datapic = $datafold & "\" & $IDD & "_image.png"
															If FileExists($datapic) Then
																$gamepic = $gamefold & "\Folder.png"
																If Not FileExists($gamepic) Then
																	Sleep(1000)
																	GUICtrlSetData($Label_percent, "cover")
																	FileCopy($datapic, $gamepic)
																	_FileWriteLog($logfle, "Copied cover.", -1)
																EndIf
															EndIf
														EndIf
													Else
														; Not using an external manifest file.
														If $gamepic <> "" Then
								...
								FileWriteLine($logfle, "")
							EndIf
							If ($f <> $files[0] And $start = "") Or $start = 1 Then
								If GUICtrlRead($Checkbox_cancel) = $GUI_CHECKED Then
					...
					If FileExists($results) Then
						...
						For $s = 1 To $sections[0]
							...
							;IniWrite($results, $file, "download", "missing")
							;
							; Consider further the idea of writing to both files for DOWNLOAD ALL.
							If $start = 1 Then
								FileWriteLine($downlog, $entry)
							Else
								FileWriteLine($histfile, $entry)
							EndIf
						Next
					...
					If $shutdown <> "none" And $cancel = "" Then
						Local $code
						If $start = 1 Then
							; Shutdown for DOWNLOAD ALL, only occurs when the STOP session is reached.
							If ($processed = $endgame And $session = $cease) Or $final = 1 Then
								; Time to query shutdown.
								_FileWriteLog($logfle, "Shutdown option enabled.", -1)
							Else
								IniWrite($inifle, "Download ALL", "shutdown", $shutdown)
								$shutdown = "none"
							EndIf
						EndIf
						If $shutdown <> "none" Then
							...
							If $ans = 1 Or $ans = -1 Then
								If $start = 1 Then
									; Finalize records.
									$completed = $completed + $processed
									;_ReplaceStringInFile($alldown, $titleID, "DONE|" & $titleID, 0, 1)
									_ReplaceStringInFile($alldown, $titleIDup, $titleID & "|DONE", 0, 1)
									_FileWriteLog($logfle, "Games processed = " & $completed, -1)
									_FileWriteLog($logfle, "Sessions completed = " & $session, -1)
									_FileWriteLog($logfle, "Download ALL Stopped.", -1)
									FileWriteLine($logfle, "")
								EndIf
								...
								Exit
							Else
								If $start = 1 Then IniWrite($inifle, "Download ALL", "shutdown", "")
							EndIf
						EndIf
					EndIf
					GUICtrlSetState($Button_download, $GUI_ENABLE)
				...
				EndIf
				If $start = 1 Then
					; Ensure the process closes the window.
					$start = "next"
				EndIf
			EndIf
		...
		Case $msg = $ListView_files Or $msg > $lastid
			...
			If IsArray($idx) Then
				...
				If $idx[0] > 0 Then
					...
					EndIf
					$entry = _GUICtrlListView_GetItemText($ListView_files, $idx, 3)
					$filetit = IniRead($downfiles, $entry, "title", "")
					GUICtrlSetData($Label_title, $filetit)
					$URL = IniRead($downfiles, $entry, "URL", "")
					GUICtrlSetData($Label_URL, $URL)
					$titleD = IniRead($downfiles, $entry, "game", "")
					GUICtrlSetData($Label_game, $titleD)
				EndIf
			EndIf
...
Func GameDetailsGUI()
	...
	Local $Group_changelog, $Group_descript, $Group_save, $Input_ID, $Input_URL, $Label_ID, $Label_URL
	...
	;
	; CONTROLS
	$Label_ID = GUICtrlCreateLabel("Game ID", 10, 10, 60, 20, $SS_CENTER + $SS_CENTERIMAGE + $SS_SUNKEN)
	GUICtrlSetBkColor($Label_ID, $COLOR_BLACK)
	GUICtrlSetColor($Label_ID, $COLOR_WHITE)
	$Input_ID = GUICtrlCreateInput("", 70, 10, 137, 20, $ES_READONLY)
	GUICtrlSetTip($Input_ID, "GOG Game ID!")
	$Button_ID = GuiCtrlCreateButton("COPY", 207, 9, 53, 23)
	GUICtrlSetFont($Button_ID, 7, 600, 0, "Small Fonts")
	GUICtrlSetTip($Button_ID, "Copy the Game ID to clipboard!")
	;
	$Label_URL = GUICtrlCreateLabel("URL", 10, 35, 38, 20, $SS_CENTER + $SS_CENTERIMAGE + $SS_SUNKEN)
	GUICtrlSetBkColor($Label_URL, $COLOR_BLUE)
	GUICtrlSetColor($Label_URL, $COLOR_WHITE)
	$Input_URL = GUICtrlCreateInput("", 48, 35, 159, 20, $ES_READONLY)
	GUICtrlSetTip($Input_URL, "GOG Game Page URL!")
	$Button_URL = GuiCtrlCreateButton("COPY", 207, 34, 53, 23)
	GUICtrlSetFont($Button_URL, 7, 600, 0, "Small Fonts")
	GUICtrlSetTip($Button_URL, "Copy the Game Page URL to clipboard!")
	;
	$Button_descript = GuiCtrlCreateButton("DESCRIPTION", 10, 65, 120, 47)
	GUICtrlSetFont($Button_descript, 9, 600)
	GUICtrlSetTip($Button_descript, "Return game description!")
	$Group_descript = GuiCtrlCreateGroup("", 10, 105, 120, 40)
	$Checkbox_descript = GUICtrlCreateCheckbox("Save description", 23, 117, 100, 20)
	GUICtrlSetTip($Checkbox_descript, "Save the game description locally!")
	;
	$Button_changelog = GuiCtrlCreateButton("CHANGELOG", 140, 65, 118, 47)
	GUICtrlSetFont($Button_changelog, 9, 600)
	GUICtrlSetTip($Button_changelog, "Return game changelog!")
	$Group_changelog = GuiCtrlCreateGroup("", 140, 105, 118, 40)
	$Checkbox_changelog = GUICtrlCreateCheckbox("Save changelog", 152, 117, 95, 20)
	GUICtrlSetTip($Checkbox_changelog, "Save the game changelog locally!")
	;
	$Group_save = GuiCtrlCreateGroup("Save To Folder Locations", 10, 150, 248, 47)
	$Checkbox_local = GUICtrlCreateCheckbox("Program Sub-folder", 20, 167, 110, 20)
	GUICtrlSetTip($Checkbox_local, "Save to a program sub-folder!")
	$Checkbox_game = GUICtrlCreateCheckbox("Game", 140, 167, 50, 20)
	GUICtrlSetTip($Checkbox_game, "Save to a game folder!")
	$Checkbox_games = GUICtrlCreateCheckbox("Games", 198, 167, 50, 20)
	GUICtrlSetTip($Checkbox_games, "Save to the games folder!")
	;
	$Checkbox_purge = GUICtrlCreateCheckbox("Sanitize Saves", 14, 205, 100, 20)
	GUICtrlSetTip($Checkbox_purge, "Sanitize (cleanup) text in the saved files!")
	;
	$Checkbox_offline = GUICtrlCreateCheckbox("Use Offline", 118, 205, 70, 20)
	GUICtrlSetTip($Checkbox_offline, "Use offline saves if they exist!")
	;
	$Button_general = GuiCtrlCreateButton("GENERAL", 10, 230, 85, 30)
	GUICtrlSetFont($Button_general, 8, 600)
	GUICtrlSetTip($Button_general, "Return general details!")
	;
	$Button_manifest = GuiCtrlCreateButton("MANIFEST", 105, 230, 85, 30)
	GUICtrlSetFont($Button_manifest, 8, 600)
	GUICtrlSetTip($Button_manifest, "View the game entry in the manifest!")
	;
	$Button_all = GuiCtrlCreateButton("ALL", 198, 208, 60, 32)
	GUICtrlSetFont($Button_all, 9, 600)
	GUICtrlSetTip($Button_all, "Return ALL details!")
	;
	$Button_close = GuiCtrlCreateButton("EXIT", 198, 250, 60, 50, $BS_ICON)
	GUICtrlSetTip($Button_close, "Exit / Close / Quit the window!")
	;
	$Button_cdkey = GuiCtrlCreateButton("CDKey CHECK && FIX", 10, 270, 180, 30)
...
Func OptionsWindow()
	...
	Local $Label_info, $Radio_one, $Radio_2, $Radio_3, $Radio_4, $Radio_5, $Radio_6, $Radio_7, $Radio_8, $Radio_9, $Radio_10
	Local $Radio_all, $Radio_extras, $Radio_most, $Radio_patch, $Radio_setup
	;
	...
	GUICtrlSetTip($Button_close, "Exit / Close / Quit the window!")
	;
	$Label_info = GUICtrlCreateLabel(@LF & "Download ALL options are also governed by your normal program settings." & @LF & @LF & _
		"WARNING - If 'Setup' or 'Patch' are not declared as usual in a filename," & @LF & _
		"then that download might be skipped in those 'File Type' case selections.", 10, 190, 370, 80, $SS_CENTER + $SS_SUNKEN)
	GUICtrlSetBkColor($Label_info, $COLOR_GREEN)
	GUICtrlSetColor($Label_info, $COLOR_WHITE)
	;
	; SETTINGS
	GUICtrlSetImage($Button_close, $user, $icoX, 1)
	;
...
Func GetFileDownloadDetails($listview = "")
	...
	For $l = 1 To $lines[0]
		...
		ElseIf StringInStr($line, '"Language":') > 0 Then
			...
			$line = StringSplit($line, '",', 1)
			$language = $line[1]
			; Language Check for external manifest file entries (in particular).
			If $language <> "" Then
				If $language <> $lang And $language <> $second Then
					$alias = ""
					$checksum = ""
					$col3 = ""
					$col4 = ""
		...
		ElseIf StringInStr($line, '"Checksum":') > 0 Then
			...
			If $exists = 1 Then
				If $ping > 0 Then
					...
					EndIf
				EndIf
				If $verify = 4 And $ratify = 4 And $allgames = "" Then
					; Check to skip existing in Database.
			...
			If $proceed = 1 Then
				...
				If IniRead($downfiles, $col4, "file", "") <> $col4 Then
					; Language Check for external manifest file entries (in particular).
					If $languages <> "" Then
						If StringInStr($languages, $lang) < 1 And StringInStr($languages, $second) < 1 Then
							$alias = ""
							$checksum = ""
							$col3 = ""
							$col4 = ""
					...
					If $allgames = 1 Then
						$fext = StringRight($col4, 3)
						If $SYS <> "Linux Mac Windows" Then
							If $SYS = "Linux" Then
								If $fext = "exe" Or $fext = "bin" Or $fext = "dmg" Or $fext = "pkg" Then ContinueLoop
							ElseIf $SYS = "Mac" Then
								If $fext = "exe" Or $fext = "bin" Or $fext = ".sh" Then ContinueLoop
							ElseIf $SYS = "Windows" Then
								If $fext = "dmg" Or $fext = "pkg" Or $fext = ".sh" Then ContinueLoop
							ElseIf $SYS = "Linux Mac" Then
								If $fext = "exe" Or $fext = "bin" Then ContinueLoop
							ElseIf $SYS = "Linux Windows" Then
								If $fext = "dmg" Or $fext = "pkg" Then ContinueLoop
							ElseIf $SYS = "Mac Windows" Then
								If $fext = ".sh" Then ContinueLoop
							EndIf
						EndIf
						If $typs <> "all" Then
							If $typs = "extras" Then
								If $col2 <> "EXTRA" Then ContinueLoop
								;If StringInStr($col4, "patch_") > 0 Or StringInStr($col4, "setup_") > 0 Then ContinueLoop
							ElseIf $typs = "most" Then
								; Don't include patch files so skip if one.
								If StringInStr($col4, "patch_") > 0 Then ContinueLoop
							ElseIf $typs = "patch" Then
								If $col2 <> "GAME" Then ContinueLoop
								If StringInStr($col4, "patch_") < 1 Then ContinueLoop
							ElseIf $typs = "setup" Then
								If $col2 <> "GAME" Then
									ContinueLoop
								Else
									If StringInStr($col4, "setup_") < 1 Then
										If $include = 1 Then
											; Include patch files.
											; NOTE - This means any files not properly named as a setup or patch will also be included.
										Else
											; Don't include patch files so skip if one.
											If StringInStr($col4, "patch_") > 0 Then ContinueLoop
										EndIf
...
Func GetManifestForTitle()
	...
	;$read = ""
	If $manall = 1 And $getlatest = 4 And $allgames = "" Then
		If FileExists($manifest) Then
			$identry = '"Id": ' & $ID & ','
			$read = FileRead($manifest)
			If StringInStr($read, $identry) > 0 Then
