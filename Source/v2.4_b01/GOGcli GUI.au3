																...
																If $date[0] > 1 Then
																	...
																	If $verify[0] > 1 Then
																		...
																		If $verify[0] > 1 Then
																			...
																			; No need to do the following, as no east to glean start time was registered in the log for validating after downloading.
																			; Date is completion time for validation. The reason for this issue, is because all downloads occur first, then validate
																			; occurs for all those downloads. NOTE - I should modify my code to write the validate start time for each file.
...
Func MainGUI()
	...
	Local $Item_manifest_entry, $Item_manifest_fix, $Item_manifest_orphan, $Item_manifest_view, $Item_validate_history
	...
	GUICtrlCreateMenuItem("", $Sub_menu_downloads)
	GUICtrlCreateMenuItem("", $Sub_menu_downloads)
	$Item_down_history = GUICtrlCreateMenuItem("History", $Sub_menu_downloads)
	GUICtrlCreateMenuItem("", $Menu_list)
	GUICtrlCreateMenuItem("", $Menu_list)
	$Sub_menu_validate = GUICtrlCreateMenu("Validation", $Menu_list)
	$Item_validate_history = GUICtrlCreateMenuItem("History", $Sub_menu_validate)
	GUICtrlCreateMenuItem("", $Menu_list)
	GUICtrlCreateMenuItem("", $Menu_list)
...
Func FileSelectorGUI()
	...
	Local $dwn, $dwnfle, $edge, $ents, $exist, $fext, $finish, $gotten, $hours, $icoDwn, $icofle, $icoUp, $IDD, $idlink, $idx, $imageD, $lastid, $mins
	Local $missing, $movdwn, $movup, $osfle, $prior, $removed, $saved, $savtxt, $secs, $sect, $sections, $SelectorGUI, $shutdown, $sizecheck, $skip
