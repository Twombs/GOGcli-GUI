...
Global $7zip, $a, $addlist, $alert, $alerts, $alf, $alldetail, $alldown, $allkeys, $alpha, $ans, $array, $backups, $bigcover, $bigpic, $blackjpg
Global $blurb, $bytes, $caption, $category, $cd, $cdkey, $cdkeys, $changelogs, $check, $checksum, $checkval, $cnt, $compare, $cookie
Global $cookies, $cover, $covers, $covimg, $declare, $descript, $descriptions, $dest, $details, $DetailsGUI, $DLC, $dlcfile, $done
Global $downfiles, $downlist, $download, $downloads, $drv, $entries, $entry, $erred, $existDB, $exists, $extbin, $extdmg, $extexe, $extpkg, $extsh, $extzip, $f, $file, $fileinfo, $filepth, $files
Global $filesize, $flag, $fold, $found, $free, $game, $gamefold, $gamelist, $gamepic, $games, $gamesfold, $gamesini, $gametxt, $getlatest
Global $gmefold, $gmesfld, $gogcli, $GOGcliGUI, $hash, $head, $height, $histfile, $history, $htmlfle, $i, $icoD, $icoF, $icoI, $icoS, $icoT, $icoW, $icoX
...
Global $purge, $r, $rat, $ratify, $read, $record, $relax, $reportexe, $res, $results, $ret, $return, $row, $s, $same, $savlog, $second, $selector
Global $SetupGUI, $shell, $size, $slug, $slugF, $slugfld, $space, $splash, $state, $style, $subfold, $tag, $tagfle, $tail, $text, $title
Global $titleF, $titlist, $top, $type, $types, $updated, $updates, $URL, $user, $validate, $valhistory, $verify, $warn, $web, $which, $width, $winpos
...
If Not FileExists($valhistory) Then
	Local $amount, $block, $c, $date, $finished, $hours, $mins, $secs, $started, $taken
...
Func MainGUI()
	...
	Local $Item_manifest_entry, $Item_manifest_fix, $Item_manifest_orphan, $Item_manifest_view, $Item_validate_histfle, $Item_validate_history
	Local $Sub_menu_alerts, $Sub_menu_comparisons, $Sub_menu_database, $Sub_menu_downloads, $Sub_menu_exclude, $Sub_menu_lists
	Local $Sub_menu_manifest, $Sub_menu_manifests, $Sub_menu_updated, $Sub_menu_validate
	;
	Local $Sub_menu_downall, $Item_downall_clear, $Item_downall_create, $Item_downall_disable, $Item_downall_display, $Item_downall_opts, $Item_downall_start, $Item_downall_view, $downall
	;
...
;~ 										EndIf
		...
		Case $msg = $Item_downall_start
			; DOWNLOAD ALL - Start
			; Start the actual downloading (same as clicking the DOWNLOAD ALL button).
		Case $msg = $Item_downall_opts
			; DOWNLOAD ALL - Options
			; Small GUI with a few options.
			; Number of games to download in a session.
			; Time delay between sessions.
			; Stop at session number (optional).
...
Func FileSelectorGUI()
	...
	Local $mins, $missing, $movdwn, $movup, $osfle, $prior, $removed, $saved, $savtxt, $secs, $sect, $sections, $SelectorGUI, $shutdown, $sizecheck, $skip
	Local $slugD, $speed, $styles, $sum, $taken, $theme, $titleD, $tmpman, $up, $upfle, $val, $valfile, $valid, $wide
	;
