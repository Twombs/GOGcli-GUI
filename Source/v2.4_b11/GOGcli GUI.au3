...
Func MainGUI()
	...
	Local $proceed, $processed, $query, $red, $rep, $result, $retrieve, $savtxt, $sect, $sects, $serial, $skipped, $slugD
	Local $tagtxt, $tested, $titleD, $valfold, $valnow, $values, $xpos, $yellow, $ypos
	;
				...
				If $allgames = 1 Then
					If FileExists($alldown) Then
						...
						If @error = 0 Then
							...
							_FileWriteLog($logfle, "Using DOWNLOAD ALL.", -1)
							$processed = 0
							For $a = 2 To $array[0][0]
								$title = $array[$a][0]
								$ID = $array[$a][1]
								$slug = IniRead($gamesini, $ID, "slug", "")
								MsgBox(262192, "Selection", "Title = " & $title & @LF & "ID = " & $ID & @LF & "Slug = " & $slug, 3, $GOGcliGUI)
...
;~ 										$caption = "Download ALL - Wallace and Gromit's Episode 1 Fright of the Bumblebees"
										...
										; Need to cater for the SHUTDOWN option, probably based on a count of games in a session.
										; $rest $stop $gams $SYS $typs $include
									Else
							...
							GUICtrlSetData($Label_bed, "Titles processed = " & $processed)
							SetStateOfControls($GUI_ENABLE, "all")
							GUICtrlSetState($Listview_games, $GUI_FOCUS)
...
Func FileSelectorGUI()
	...
	Local $skip, $slugD, $speed, $start, $styles, $sum, $taken, $theme, $titleD, $tmpman, $up, $upfle, $val, $valfile, $valid, $wide
	;
